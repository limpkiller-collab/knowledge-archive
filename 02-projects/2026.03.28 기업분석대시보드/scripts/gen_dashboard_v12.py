#!/usr/bin/env python3
"""v12: 실적발표(Earnings) 탭 추가
- 최근 4분기 실적 서프라이즈 KPI (EPS Beat/Miss)
- 분기별 컨퍼런스콜 요약 카드 (CEO/CFO 발언 + 키워드 배지)
- 다음 실적발표 예정 카드 (Q1 FY2027 가이던스)
- 애널리스트 실적 분석 코멘트
- 매출 실적 vs 컨센서스 비교 차트
- v11 Supanova Design System 유지
"""
import math
...
<div class="wm">StockLens Dashboard v12.0 | NVDA 종합분석 | 데이터: NVIDIA IR 공시 (FY2026 Q4, 2026.02.25) + 애널리스트 리포트 | 생성일: 2026.03.28<br>주요 출처: NVIDIA Newsroom, MacroTrends, FactSet Consensus | ⚠️ 투자 참고용이며 투자 권유가 아닙니다.</div>
...
# Write to file
output_path = "/sessions/exciting-gracious-pasteur/mnt/자산 대시보드 제작/02-projects/2026.03.28 기업분석대시보드/output/기업분석_종합대시보드_v12_2026.03.28.html"
with open(output_path, "w", encoding="utf-8") as f:
    f.write(html)

print(f"v12 Dashboard generated: {output_path}")
