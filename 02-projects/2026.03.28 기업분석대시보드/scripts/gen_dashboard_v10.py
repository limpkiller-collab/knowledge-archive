#!/usr/bin/env python3
"""v10: 연간/분기별 서브탭 추가 — 재무분석·사업부문
- v9 기반 + 재무분석/사업부문 탭에 연간·분기별 서브탭 토글 추가
- 분기별 세그먼트 데이터 (Q1~Q4 FY2026 IR 공시 기반) 추가
- 분기별 재무 하이라이트 코멘터리 추가
- pill-switch 스타일 서브탭 UI
"""
import math
...
/* Sub-tabs (pill switch for 연간/분기별) */
.sub-tabs{{display:inline-flex;background:var(--card2);border-radius:8px;padding:3px;margin-bottom:24px;gap:2px}}
.sub-tab{{padding:8px 20px;cursor:pointer;color:var(--text2);font-size:13px;font-weight:600;border-radius:6px;transition:all .2s;user-select:none}}
.sub-tab:hover{{color:var(--text)}}.sub-tab.active{{background:var(--accent);color:#fff}}
...
<div class="tc" id="t-segments">
  ...
  <div class="sub-pane active" id="seg-annual">
    <div class="cg">
      ...
      <div class="cc full"><div class="ct">사업부문 연간 상세 데이터</div><div class="tw">{seg_table()}</div></div>
    </div>
  </div>
  ...
  <div class="sub-pane" id="seg-quarterly">
    ...
    <div class="cg">
      ...
      <div class="cc full"><div class="ct">분기별 사업부문 상세 데이터 <span class="badge">FY2026</span></div><div class="tw">{q_seg_table()}</div></div>
    </div>
  </div>
...
<div class="wm">StockLens Dashboard v10.0 | NVDA 종합분석 | 데이터: NVIDIA IR 공시 (FY2026 Q4, 2026.02.25) + 애널리스트 리포트 | 생성일: 2026.03.28<br>주요 출처: NVIDIA Newsroom, MacroTrends, FactSet Consensus | ⚠️ 투자 참고용이며 투자 권유가 아닙니다.</div>
...
# Write to file
output_path = "/sessions/exciting-gracious-pasteur/mnt/자산 대시보드 제작/02-projects/2026.03.28 기업분석대시보드/output/기업분석_종합대시보드_v10_2026.03.28.html"
with open(output_path, "w", encoding="utf-8") as f:
    f.write(html)

print(f"v10 Dashboard generated: {output_path}")
