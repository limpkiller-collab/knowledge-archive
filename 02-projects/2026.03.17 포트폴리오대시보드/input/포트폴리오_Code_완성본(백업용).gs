...
// ── 전략별 비중 (Portfolio U47:Y55) ──────────────────────────────────────────
// [0]=전략명  [1]=매수금액  [2]=평가금액  [3]=비중  [4]=수익률
function getStrategy(ss) {
  const ws   = ss.getSheetByName(SHEET_PORTFOLIO);
  const data = ws.getRange('U47:Y55').getValues();
  const strategy = [];
  for (const row of data) {
...
function getCashflow(ss) {
  ...
  const avgFxRate  = Number(ws.getRange('L10').getValue() || 0);
  const fxGainLoss = Number(ws.getRange('M11').getValue() || 0);
...
function addTransactionRow(ss, d) {
  ...
  ws.getRange(lastRow, 8).setFormula(`=J${lastRow}*L${lastRow}`);
  ...
  ws.getRange(lastRow, 14).setFormula(`=$K${lastRow}/$F${lastRow}`);
}
...
function fetchAndStoreNews() {
  const ss      = SpreadsheetApp.getActiveSpreadsheet();
  const tickers = getHoldings(ss)
    .filter(h => h.sector1 !== 'ETF' && h.sector1 !== 'Bond')
    .map(h => h.ticker).filter(Boolean);

  const ws = ss.getSheetByName(SHEET_NEWS) || ss.insertSheet(SHEET_NEWS);
  ws.clearContents();
  ws.getRange(1, 1, 1, 5).setValues([['Ticker','Date','Title','Summary','Link']]);
  ...
  for (const ticker of tickers) {
    try {
      ...
      for (const item of items) {
        ...
        try {
          if (title)   title   = LanguageApp.translate(title, 'en', 'ko');
          if (summary) summary = LanguageApp.translate(summary.slice(0, 200), 'en', 'ko');
        } catch(te) {}

        const dateStr = pubDate ? Utilities.formatDate(pubDate, 'Asia/Seoul', 'yyyy-MM-dd') : '';
        rows.push([ticker, dateStr, title, summary, item.link || '']);
        count++;
      }
      Utilities.sleep(400);
    } catch(e) {
      Logger.log(`뉴스 로드 실패 (${ticker}): ${e.message}`);
    }
  }

  if (rows.length > 0) ws.getRange(2, 1, rows.length, 5).setValues(rows);
  Logger.log(`✅ 뉴스 업데이트 완료: ${rows.length}건 / ${tickers.length}개 종목`);
}
...
function parseRSSItems_(xmlText) {
  ...
  for (const chunk of chunks) {
    ...
    items.push({ title: rawTitle, link: get('link').trim(), pubDate: get('pubDate').trim(), summary: rawSummary.slice(0, 300) });
  }
  return items;
}
